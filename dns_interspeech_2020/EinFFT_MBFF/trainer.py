...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        if self.rank == 0:
            avg_train_loss = loss_total / len(self.train_dataloader)
    ...
    def _validation_epoch(self, epoch):
        ...
        # 输出 "With_reverb" 和 "No_reverb" 的平均损失
        for speech_type in ("With_reverb", "No_reverb"):
            avg_speech_loss = loss_list[speech_type] / item_idx_list[speech_type]
            print(f"Epoch {epoch} - {speech_type} Loss: {avg_speech_loss:.4f}")
            self.writer.add_scalar(
                f"Loss/{speech_type}",
                avg_speech_loss,
                epoch,
            ...
            print(f"Epoch {epoch} - {speech_type} Validation Score: {validation_score_list[speech_type]:.4f}")
